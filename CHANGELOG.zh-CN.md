...
本文件记录「切换应用」的所有重要变更；上游 Cockpit Tools 的历史记录仅作为移植基线保留。
...
- **修复 WorkBuddy 认证写入失败后的状态残留**：写入失败会回滚，避免留下半保存的认证快照。
- **修复 WorkBuddy 窗口激活失败被误报为切换失败**：现在分别报告账号切换结果和窗口激活结果。
- **修复 WorkBuddy 卡片 metrics 结构不匹配**：统一前后端数据契约，恢复积分和状态信息的正确展示。
- **修复 Work CN 会话监听器异常后的资源清理问题**：降低重复监听和后续状态错乱风险。
- **补充关键回归测试并同步前端锁文件版本**：覆盖切换生命周期、认证回滚、卡片数据和令牌轮换逻辑。
...
- 日常积分签到继续由独立的 [daily-checkin](https://github.com/lk1015646426/daily-checkin) GitHub Actions 仓库负责，本项目不新增签到工作流。
- 本版本不包含账号快照、Token、私钥、日志、数据库或构建产物。
